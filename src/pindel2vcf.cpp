...
public:
    IntParameter(int *par_ptr, const string &shortName, const string &longName, const string &description,
...
IntParameter::IntParameter(int *par_ptr, const string &shortName, const string &longName, const string &description,
                           const bool required, const int value) :
...
public:
    BoolParameter(bool *par_ptr, const string &shortName, const string &longName, const string &description,
...
BoolParameter::BoolParameter(bool *par_ptr, const string &shortName, const string &longName, const string &description,
                             const bool required, const bool value) :
...
public:
    FloatParameter(double *par_ptr, const string &shortName, const string &longName, const string &description,
...
FloatParameter::FloatParameter(double *par_ptr, const string &shortName, const string &longName,
                               const string &description, const bool required, const double value) :
        Parameter(shortName, longName, description, required) {
    d_data_ptr = par_ptr;
    *d_data_ptr = value;
}
...
public:
    StringParameter(string *par_ptr, const string &shortName, const string &longName, const string &description,
...
StringParameter::StringParameter(string *par_ptr, const string &shortName, const string &longName,
                                 const string &description, const bool required, const string &value) :
        Parameter(shortName, longName, description, required) {
    d_data_ptr = par_ptr;
    *d_data_ptr = value;
}
...
void Chromosome::readFromFile() {
    ...
    do {
        ...
        } while (counter < refLine.size() && (refLine[counter] != ' ') && (refLine[counter] != '\t') &&
                 (refLine[counter] != '\n'));
...
void createHeader(ofstream &outFile, const string &sourceProgram, const string &reference, const set<string> &samples) {
    ...
    outFile
            ...
            << endl;
    outFile << "##INFO=<ID=XQS,Number=1,Type=Integer,Description=\"Phred Quality score of chi-square test\">"
            << endl;
    outFile
    ...
    outFile << "##FORMAT=<ID=AD,Number=2,Type=Integer,Description=\"Allele depth, how many reads support this allele\">"
            << endl;
...
public:
    ...
    bool isEquilengthReplacement() const {
        if ((d_svtype == "RPL" && d_svlen == d_replaceLen) ||
...
double fisher_test(int a, int c, int b, int d) {
    ...
    p = (FACT(a + b) * FACT(c + d) * FACT(a + c) * FACT(d + b)) /
        (double) (FACT(a) * FACT(b) * FACT(c) * FACT(d) * FACT(n));
    return p;
}
...
double chisqr_test(int a, int c, int b, int d) {
    ...
    x2 = pow((a - theoretical_a), 2.0) / theoretical_a + pow((b - theoretical_b), 2.0) / theoretical_b +
         pow((c - theoretical_c), 2.0) / theoretical_c + pow((d - theoretical_d), 2.0) / theoretical_d;
    return x2;
...
//    }
    if (svd.d_format.size() == 2) {
        somatic_p_value = chisqr2pValue(1, chisqr_test(svd.d_format[0].getTotalRefSupport(),
                                                       svd.d_format[1].getTotalRefSupport(),
                                                       svd.d_format[0].getTotalReads(),
                                                       svd.d_format[1].getTotalReads()));
    ...
    if (svd.d_svtype.compare("RPL") == 0 || svd.d_svtype.compare("DUP:TANDEM") == 0 ||
        svd.d_svtype.compare("INV") == 0) {
...
void
getSampleNamesAndChromosomeNames(InputReader &pindelInput, set<string> &sampleNames, set<string> &chromosomeNames) {
...
void convertIndelToSVdata(InputReader &pindelInput, map<string, int> &sampleMap, Genome &genome, SVData &svd,
                          const string &targetChromosomeID) {
    string line;
    ...
    if (svType.compare("LI") == 0) {
        ...
        int sampleMinSupport = atoi(fetchElement(lineStream,
                                                 numberItemsUntilNextSupport).c_str()); // now at position 35, total +supports sample 1
        ...
        while (!lineStream.fail()) {
          ...
          refSupportAtEndOfEvent = atoi( fetchElement( lineStream, 1 ).c_str());
            ...
            samplePlusSupport = atoi(
                    fetchElement(lineStream, numberItemsUntilNextSupport).c_str()); // for unique support, 2->1
            sampleMinSupport = atoi(fetchElement(lineStream,
                                                 numberItemsUntilNextSupport).c_str()); // now at position 33, total +supports sample 1
        }
        return;
    }
    ...
    int plusSupport = atoi(fetchElement(lineStream, numberOfItemsUntilNextSupport -
                                                    1).c_str()); // now at position 33, total +supports sample 1; for unique support 1->2
    int minSupport = atoi(fetchElement(lineStream,
                                       numberOfItemsUntilNextSupport).c_str()); // now at position 35, total +supports sample 1
    ...
    while (!lineStream.fail()) {
        ...
        plusSupport = atoi(
                fetchElement(lineStream, numberOfItemsUntilNextSupport - 1).c_str()); // for unique support, 2->1
        minSupport = atoi(fetchElement(lineStream,
                                       numberOfItemsUntilNextSupport).c_str()); // now at position 33, total +supports sample 1
    }
}
...
void createParameters() {
    parameters.push_back(
            new StringParameter(&g_par.reference, "-r", "--reference",
                                "The name of the file containing the reference genome", true, ""));
    parameters.push_back(
            new StringParameter(&g_par.referenceName, "-R", "--reference_name",
                                "The name and version of the reference genome", true, ""));
    parameters.push_back(
            new StringParameter(&g_par.referenceDate, "-d", "--reference_date",
                                "The date of the version of the reference genome used", true, ""));
    parameters.push_back(
            new StringParameter(&g_par.pindelfile, "-p", "--pindel_output",
                                "The name of the pindel output file containing the SVs", false, ""));
    parameters.push_back(
            new StringParameter(&g_par.pindelroot, "-P", "--pindel_output_root",
                                "The root-name of the pindel output file; this will result in\n"
    ...
    parameters.push_back(
            new StringParameter(&g_par.vcffile, "-v", "--vcf",
                                "The name of the output vcf-file (default: name of pindel output file +\".vcf\"", false,
                                ""));
    parameters.push_back(
            new StringParameter(&g_par.chromosome, "-c", "--chromosome",
                                "The name of the chromosome (default: SVs on all chromosomes are processed)", false,
                                ""));
    parameters.push_back(
            new IntParameter(&g_par.windowSize, "-w", "--window_size",
                             ...
                             false, 300));
    parameters.push_back(
            new IntParameter(&g_par.MinCoverage, "-mc", "--min_coverage",
                             "The minimum number of reads to provide a genotype (default 10)", false, 10));
    parameters.push_back(
            new FloatParameter(&g_par.HetCutoff, "-he", "--het_cutoff",
                               "The propertion of reads to call het (default 0.2)", false, 0.2));
    parameters.push_back(
            new FloatParameter(&g_par.HomCutoff, "-ho", "--hom_cutoff",
                               "The propertion of reads to call het (default 0.8)", false, 0.8));

    parameters.push_back(
            new IntParameter(&g_par.minsize, "-is", "--min_size",
                             "The minimum size of events to be reported (default 1)", false, 1));
    parameters.push_back(
            new IntParameter(&g_par.maxsize, "-as", "--max_size",
                             "The maximum size of events to be reported (default infinite)", false, -1));
    parameters.push_back(
            new BoolParameter(&g_par.bothstrands, "-b", "--both_strands_supported",
                              "Only report events that are detected on both strands (default false)", false, false));
    parameters.push_back(
            new IntParameter(&g_par.minsuppSamples, "-m", "--min_supporting_samples",
                             "The minimum number of samples an event needs to occur in with sufficient support to be reported (default 0)",
                             false, 1));
    parameters.push_back(
            new IntParameter(&g_par.minsuppReads, "-e", "--min_supporting_reads",
                             "The minimum number of supporting reads required for an event to be reported (default 1)",
                             false, 1));
    parameters.push_back(
            new IntParameter(&g_par.maxSuppReads, "-f", "--max_supporting_reads",
                             ...
                             false, -1));
    parameters.push_back(
            new IntParameter(&g_par.regionStart, "-sr", "--region_start",
                             "The start of the region of which events are to be reported (default 0)", false, 0));
    parameters.push_back(
            new IntParameter(&g_par.regionEnd, "-er", "--region_end",
                             "The end of the region of which events are to be reported (default infinite)", false, -1));
    parameters.push_back(
            new IntParameter(&g_par.maxInterRepeatNo, "-ir", "--max_internal_repeats",
                             ...
                             false, -1));
    parameters.push_back(
            new IntParameter(&g_par.compactOutput, "-co", "--compact_output_limit",
                             ...
                             false, g_sizeToWarnFor));
    parameters.push_back(
            new IntParameter(&g_par.maxInterRepeatLength, "-il", "--max_internal_repeatlength",
                             ...
                             false, -1));
    parameters.push_back(
            new IntParameter(&g_par.maxPostRepeatNo, "-pr", "--max_postindel_repeats",
                             ...
                             false, -1));
    parameters.push_back(
            new IntParameter(&g_par.maxPostRepeatLength, "-pl", "--max_postindel_repeatlength",
                             ...
                             false, -1));
    parameters.push_back(
            new BoolParameter(&g_par.onlyBalancedSamples, "-sb", "--only_balanced_samples",
...
//   parameters.push_back(
    ...
    parameters.push_back(
            new IntParameter(&g_par.minimumStrandSupport, "-ss", "--minimum_strand_support",
                             "Only count a sample as supporting an event if at least one of its strands is supported by X reads (default 1)",
                             false, 1));
    parameters.push_back(
            new BoolParameter(&g_par.gatkCompatible, "-G", "--gatk_compatible",
    ...
    for (int argumentIndex = 1; argumentIndex < argc; argumentIndex++) {
        ...
        if (parameters[parameterIndex]->isUnary()) {
            ...
            if ((argumentIndex + 1 < argc) &&
                (argv[argumentIndex + 1][0] != '-')) { // so there are more arguments, and next one isn't regular -x
...
bool checkParameters() {
    ...
    for (int parameterIndex = 0; parameterIndex < parameters.size(); parameterIndex++) {
        if (parameters[parameterIndex]->isRequired() && !parameters[parameterIndex]->isSet()) {
            cout << "\nRequired parameter " << parameters[parameterIndex]->getShortName() << "/"
    ...
    if (!canRun) {
        cout << "For further information, please run " << g_programName
             << " without arguments or with option -h/--help.\n\n";
    }
    return canRun;
}
...
bool throughFilter(SVData sv) {
    ...
    if ((g_par.minsuppSamples >= 1) &&
        (sv.getNumSupportSamples(g_par.onlyBalancedSamples, g_par.minimumStrandSupport) < g_par.minsuppSamples)) {
        return false;
    }
    ...
    if (g_par.maxInterRepeatNo >= 0 &&
        !sv.withinAllowedRepeatsInternal(g_par.maxInterRepeatLength, g_par.maxInterRepeatNo)) {
        return false;
    }
    if (g_par.maxPostRepeatNo >= 0 &&
        !sv.withinAllowedRepeatsPostIndel(g_par.maxPostRepeatLength, g_par.maxPostRepeatNo)) {
        return false;
    }
...
void reportSVsInChromosome(
    ...
    do {
        ...
        while (!pindelInput.eof()) {
            ...
            if (!pindelInput.eof() && (chromosomeID == "" ||
                                       (svd.getChromosome() == chromosomeID && svd.getPosition() >= regionStart &&
                                        svd.getPosition() < regionEnd))) {
                svs.push_back(svd);
            }
            counter++;
...
int main(int argc, char *argv[]) {
    ...
    if (g_par.chromosome != "") {
        ...
        for (int chromosomeCount = 0; chromosomeCount < genome.d_chromosomes.size(); chromosomeCount++) {
            reportSVsInChromosome(genome.d_chromosomes[chromosomeCount].getID(), chromosomeNames, sampleNames,
                                  pindelInput, sampleMap, genome, vcfFile);
